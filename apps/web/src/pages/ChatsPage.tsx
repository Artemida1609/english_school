...
const chats = [
  {
    ...
    lastMessage: "Чудова робота з вчорашнім завданням!",
    time: "09:41",
    unread: 2,
    online: true,
  },
  {
    ...
    lastMessage: "Хтось може пояснити Present Perfect?",
    time: "Вчора",
    unread: 14,
    online: false,
  },
...
export const ChatsPage = () => {
  ...
  // На мобільному показуємо або sidebar, або чат
  ...
  return (
    <section className="flex h-full rounded-none sm:rounded-2xl overflow-hidden
      h-[calc(100vh-4rem)] overflow-hidden
      border-0 sm:border border-slate-100 bg-white sm:shadow-sm sm:mx-0 sm:mx-2">
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 sm:px-4 py-3
                ...
                ${activeChat.id === chat.id
                  ? "bg-emerald-50/70 border-r-2 border-emerald-400"
                  : ""}`}
            >
              {/* Avatar */}
              <div className="relative flex-shrink-0">
                <div className={`w-11 h-11 rounded-2xl bg-gradient-to-br ${chat.bg}
      ...
      `}>
        ...
        <header className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3
          ...
          <button
            ...
            className="md:hidden w-8 h-8 flex items-center justify-center
              rounded-xl hover:bg-slate-100 transition-colors text-slate-500 flex-shrink-0">
          ...
          {/* Avatar */}
          <div className="relative flex-shrink-0">
            <div className={`w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-gradient-to-br ${activeChat.bg}
          ...
          {/* Name */}
          <div className="flex-1 min-w-0">
            <h2 className="text-sm font-bold text-slate-900 truncate">{activeChat.name}</h2>
            <p className="text-xs text-slate-400 truncate hidden sm:block">{activeChat.subtitle}</p>
          </div>
          ...
          <div className="flex items-center gap-0.5 sm:gap-1 flex-shrink-0">
            ...
            <button className="w-8 h-8 flex items-center justify-center
              rounded-xl hover:bg-slate-100 transition-colors text-slate-400 hover:text-slate-600">
          ...
          {currentMessages.map((msg, i) => (
            ...
            >
              <div className={`max-w-[85%] sm:max-w-[75%] md:max-w-[65%]
                px-3 sm:px-3.5 py-2 sm:py-2.5 rounded-2xl
                ${msg.mine
                ...
                <p className={`text-[10px] mt-1 text-right
                  ${msg.mine ? "text-white/70" : "text-slate-400"}`}>
                  {msg.time}
                </p>
              </div>
            </motion.div>
        ...
        {/* Input */}
        <footer className="px-2 sm:px-3 py-2 sm:py-3 border-t border-slate-100
          bg-white flex-shrink-0">
          <div className="flex items-center gap-2 bg-slate-50 rounded-xl sm:rounded-2xl
            px-3 sm:px-4 py-2 sm:py-2.5
            border border-slate-100 focus-within:border-emerald-300
            ...
            <button
              ...
              className="w-7 h-7 sm:w-8 sm:h-8
                ...
                disabled:opacity-40 hover:shadow-md hover:shadow-emerald-200
